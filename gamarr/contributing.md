---
title: Gamarr Contributing
description: Guide for contributing to Gamarr development including setup, building, testing, and submitting changes
published: true
date: 2025-02-02T00:00:00.000Z
tags: gamarr, development, contributing, guide, setup
editor: markdown
...
Gamarr is written in C# (backend) and JS (frontend). The backend is built on the .NET6 framework, while the frontend utilizes Reactjs.
...
> The Application will **NOT** run on older versions such as `18.x`, `16.x` or any version below 20.0!
{.is-warning}
...
- Each PR should come from its own [feature branch](https://martinfowler.com/bliki/FeatureBranch.html) not develop in your fork, it should have a meaningful branch name (what is being added/fixed)
  - `new-feature` (Good)
  - `fix-bug` (Good)
  - `patch` (Bad)
  - `develop` (Bad)
...
From VS simply navigate to Test Explorer and run or debug the tests you would like to examine.

Tests can either be run all at once or one at a time in VS.

From command line the `test.sh` script accepts 3 parameters

```bash
test.sh <TESTNAME> <PLATFORM> <COVERAGE>
```

### Writing Tests

While not always fun, we encourage writing unit tests for any backend code changes. This will ensure the change is functioning as you intended and that future changes to don't break expected behavior.

> We require 80% coverage of new code when submitting a PR
{.is-info}
...
Gamarr uses a self hosted open access [Weblate](https://translate.servarr.com) instance for managing its json translation files. These files are stored in the repo at `src/NzbDrone.Core/Localization`
...
The English translation, `en.json`, serves as the source for all other translations and is managed on GitHub repo. When adding a new string to either the UI or backend a key must also be added to `en.json` along with the default value in English. This default key may then be consumed as follows:

### Backend Strings

Backend strings may be added by utilizing the Localization Service `GetLocalizedString` method
...
var message = _localizationService.GetLocalizedString("IndexerHealthCheckNoIndexers")
```
...
<div>
  {translate('UnsupportedQualityProfile')}
</div>
```

# Thank You

Finally, a huge thanks to all our existing contributors and to you for taking the time to contribute to Gamarr. Your efforts are greatly appreciated.
